...
import numpy as np
from time_causal_vae.evaluation.external.awd.normal_ot import solve_sinkhorn, gurobi_2d, solve_pot, solve_unbalanced

# from gurobipy import *
from time import time

# from awd.measures import get_meas_for_sinkhorn, get_joint_prob, get_full_index_markov, get_start_next_indices
...
# @profile
def solve_dynamic_old(cost, mu, nu, supp_mu, supp_nu, g, index_mu=0, index_nu=0, outputflag=1, method="gurobi"):
    """
    ...
    for i_t in range(T):
        ...
        if N_h_nu == N_h_mu == 0:
            ...
            for i in range(N_h_mu):
                ...
                input_mu = mu(
                    v_here, p_par_mu_h
                )  # we compute this N x N many times --> only needs to be computed N many times
                mu_list_h.append(input_mu)

                # compute dinstegrations for mu
            nu_list_h = []
            for j in range(N_h_nu):
                # compute distingerations for nu
                p_nu_h = supp_h_nu[j, :]
                p_nu_h_ext = np.zeros(T)
                p_nu_h_ext[rel_nodes] = p_nu_h
            ...
            for i in range(N_h_mu):
                for j in range(N_h_nu):
                    ...
                    # # for each pair of support points, get the disintegrations (i.e. measures) that are relevant for the
                    # # OT problem
                    # input_mu = mu(v_here, p_par_mu_h)  # we compute this N x N many times --> only needs to be computed N many times
                    # input_nu = nu(v_here, p_par_nu_h)
                    input_mu = mu_list_h[i]
                    input_nu = nu_list_h[j]
...
def gurobi_bm(margs, f, p_dist=2, radial_cost=0, f_id=0, minmax="min", r_opti=0, outputflag=1, causal=0, anticausal=0):
    ...
    m.addConstrs((pi_var.sum(i, "*") == pl_1[i]
                 for i in range(n1)), name="first_marg")
    m.addConstrs((pi_var.sum("*", i) == pl_2[i]
                 for i in range(n2)), name="second_marg")

    # add causal constraint: (Note: doesn't seem very efficient, but not sure how else to do)
    causal_count = 0
    if causal == 1:
        for t in range(1, n_dim):
            x_t_arr, ind_inv = np.unique(
                xl_1[:, :t], axis=0, return_inverse=True)
            for ind_t in range(len(x_t_arr)):
                pos_h = np.where(ind_inv == ind_t)[0]
                y_t_arr, ind_inv_y = np.unique(
                    xl_2[:, :t], axis=0, return_inverse=True)
    ...
    if minmax == "min":
        obj = quicksum([cost_mat[i, j] * pi_var[i, j]
...
def sinkhorn_bicausal_markov(
    mu_list, nu_list, cost_list, n_list, m_list, eps_stop=10**-4, max_iter=10**4, reshape=True, outputflag=0
):
    ...
    for t in range(1, t_max):
        f_h = [[np.ones([len(mu_list[t][i][1]), 1])
                for j in range(m_list[t - 1])] for i in range(n_list[t - 1])]
        g_h = [[np.ones([1, len(nu_list[t][j][1])])
                for j in range(m_list[t - 1])] for i in range(n_list[t - 1])]
        c_f_h = [[1 for j in range(m_list[t - 1])]
                 for i in range(n_list[t - 1])]
        c_g_h = [[1 for j in range(m_list[t - 1])]
                 for i in range(n_list[t - 1])]
        f_list.append(f_h)
        g_list.append(g_h)
    ...
    if outputflag:
        print("Defining update functions took " +
              str(time() - t_funs) + " seconds")
    ...
    if reshape:
        ...
        if outputflag:
            print("Reshaping input took " +
                  str(time() - t_reshape) + " seconds")
    ...
    while iter_h < max_iter and np.abs(prev_val - value_f - value_g) > eps_stop:
        if iter_h % 10 == 0 and outputflag:
            print("Current iteration:", iter_h, "Current value:",
                  value_f + value_g, "Current time:", time() - t_solve)
        iter_h += 1
    ...
    for t_m in range(t_max):
        ...
        if t == t_max - 1:
            for i in range(n_list[t - 1]):
                for j in range(m_list[t - 1]):
                    V_t[i, j] = np.sum(
                        -np.log(cost_list[t][mu_list[t][i]
                                [0], :][:, nu_list[t][j][0]])
                        * f_list[t][i][j]
                        * g_list[t][i][j]
            ...
            value = np.sum(
                (-np.log(cost_list[0][mu_list[0][0], :][:, nu_list[0]
                 [0]]) + V_tp[mu_list[t][0], :][:, nu_list[t][0]])
                * f_list[0]
                * g_list[0]
                * cost_list[0][mu_list[0][0], :][:, nu_list[0][0]]
...
def sinkhorn_causal_markov(
    ...
    for t in range(1, t_max):
        f_h = [[np.ones([len(mu_list[t][i][1]), 1])
                for j in nu_index_full[t - 1]] for i in range(n_list[t - 1])]
        f_list.append(f_h)
    s_nu_1Tm1 = nu_index_full[-2]
    g = [np.ones([1, len(nu_list[t_max - 1][iTm1[-2]][1])])
         for iTm1 in s_nu_1Tm1]
    ...
    def full_update_f_list():
        for t_m in range(t_max):
            ...
            if t == 0:
                f_list[0], value_f = update_f_1(
                    mu_list[0][1], nu_list[0][1], cvh, cost_list[0][mu_list[0]
                                                                    [0], :][:, nu_list[0][0]]
                )
            elif t == t_max - 1:
                for i in range(n_list[t - 1]):
    ...
    while iter_h < max_iter and np.abs(prev_val - value_f - value_g) > eps_stop:
        if iter_h % 10 == 0 and outputflag:
            print("Current iteration:", iter_h,
                  "Current value:", value_f + value_g)
        iter_h += 1
        prev_val = value_f + value_g
        value_f = full_update_f_list()
        value_g = update_g()
    ...
    const_val = 0
    # iterate over y_1, ..., y_{T-1} jointly. So g(y) = g(y_{1:T-1}, y_T)
    for j, ind_tot in enumerate(itm1):
        snu_shape = np.prod(np.shape(nu_list[t_max - 1][ind_tot[-2]][1]))
        for t_m in range(t_max):
            t = t_max - t_m - 1
            if t > 1:
            ...
            for i in range(smu_shape):
                if t == t_max - 1:
                    ...
                    Vnext[i, :] = np.sum(
                        -np.log(cost_list[t][mu_list[t][i][0], :]
                                [:, nu_list[t][ind_tot[-2]][0]])
                    ...
                    Vconst[i, :] = np.sum(
                        ...
                        * cost_list[t][mu_list[t][i][0], ind_tot[2 * t]: ind_tot[2 * t] + 1]
                        * f_list[t][i][ind_tot[2 * t - 1]]
                        * mu_list[t][i][1],
                        axis=0,
                    )
                    Vnext[i, :] = np.sum(
                        (
                            Vcur[mu_list[t][i][0], :]
                            + Vconst_cur[mu_list[t][i][0], :]
                            * (-np.log(cost_list[t][mu_list[t][i][0], ind_tot[2 * t]: ind_tot[2 * t] + 1]))
                        )
                        * cost_list[t][mu_list[t][i][0], ind_tot[2 * t]: ind_tot[2 * t] + 1]
                        * f_list[t][i][ind_tot[2 * t - 1]]
                        * mu_list[t][i][1],
                    ...
                    Vconst[i, :] = np.sum(
                        ...
                        * cost_list[t][mu_list[t][0], ind_tot[2 * t]: ind_tot[2 * t] + 1]
                        * f_list[0]
                        * mu_list[t][1],
                        axis=0,
                    )
                    Vnext[i, :] = np.sum(
                        (
                            Vcur[mu_list[t][0], :]
                            + Vconst_cur[mu_list[t][i][0], :]
                            * (-np.log(cost_list[t][mu_list[t][0], ind_tot[2 * t]: ind_tot[2 * t] + 1]))
                        )
                        * cost_list[t][mu_list[t][0], ind_tot[2 * t]: ind_tot[2 * t] + 1]
                        * f_list[0]
                        * mu_list[t][1],
        ...
        value += nu_probs_tm1[ind_tot[-1]] * \
            np.sum(Vcur * nu_list[t_max - 1][ind_tot[-2]][1] * g[j])
        const_val += nu_probs_tm1[ind_tot[-1]] * \
            np.sum(Vconst_cur * nu_list[t_max - 1][ind_tot[-2]][1] * g[j])
