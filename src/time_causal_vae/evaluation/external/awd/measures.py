    ...
    if len(rel_par) == 0:
        ...
        if all_out == 0:
            ...
            all_margs = get_marginals_multi(
                mu, supp_mu, rel_par, g, given_margs, all_out=1, index_mu=index_mu, filt=filt
            )
...
# TODO: Below is not clean, should be used with care


def get_disintegration(marg, inds1, tol_decimals=6):
    # TODO: DANGER WARNING! There has been some messed up indexing here, check whether everything is still correct
    # marg is a list with two entries, one n x d np array, and one n-list of weights
    # returns the disintegrations of marg, i.e., inds1 is a subset of {1, 2, ..., d} which are the elements that we
    # condition on
    # returns margs_inds1, cond_measures, supp_inds1, supp_inds2, where supp_inds1 and supp_inds2 are n x d_1 and n x (d-d_1)
    # Set inds2 as the sorted list containing the elements in {1, 2, ..., d} that are not in inds1, d_1 = len(inds1)
    # supp_inds1 is the support of marg at the indices inds1, and similar for inds2
    # margs_inds1 is the marginal distribution at inds1
    # cond_measures is a list, for where cond_measures[i] is the conditional distribution at inds2 conditioned on supp_inds1[i]

    # get shape and tuples of indices
    marg[0] = marg[0].round(decimals=tol_decimals)
    (n, d) = marg[0].shape
    d_1 = len(inds1)
    if d_1 == 0:
        marg_inds1 = [[], []]
        ...
        supp_1 = np.zeros([0, 0])
        supp_2 = marg[0]
        return marg_inds1, cond_measures, supp_1, supp_2
    d_2 = d - d_1
    all_points = marg[0]
    all_weights = marg[1]

    inds1 = list(np.array(inds1) - 1)
    inds1.sort()
    inds2 = []
    for i in range(d):
        if i not in inds1:
    ...
    # inds1 = tuple([int(x) for x in inds1],)

    # get supports
    supp_1_pre = all_points[:, inds1]
    supp_1, inv_unique = np.unique(supp_1_pre, axis=0, return_inverse=True)

    supp_2 = all_points[:, inds2]
    supp_2 = np.unique(supp_2, axis=0)

    # get marginal at inds1
    marg_inds1 = reduce_meas([supp_1_pre, marg[1]])
    # w_new = np.zeros(len(supp_1))
    # for i in range(len(supp_1)):
    #     for l in range(len(all_weights)):
    #         if np.all(supp_1[i, :] == supp_1_pre[l, :]):
    #             w_new[i] += marg[1][l]
    # marg_inds1 = [supp_1, w_new]  # marginal distribution at inds_1

    n_cond = len(supp_1)
    cond_x = [np.zeros([0, d_2]) for _ in range(n_cond)]
    cond_w = [[] for _ in range(n_cond)]

    for j in range(n):
        ind_h = inv_unique[j]
        cond_x[ind_h] = np.append(cond_x[ind_h], all_points[j : j + 1, inds2], axis=0)
        cond_w[ind_h].append(all_weights[j])
    for i in range(n_cond):
        cond_w[i] = cond_w[i] / np.sum(cond_w[i])
    cond_measures = [[cond_x[i], cond_w[i]] for i in range(n_cond)]

    # cond_measures = []  # each entry will be one measure, conditioned on the respective element in the support of supp_1
    # for i in range(n_cond):
    #     x_1 = supp_1[i, :]
    #
    #     p_here = np.zeros([0, d_2])
    #     w_here = []
    #     # find elements in all_points that have the first coordinates equal to supp_1
    #     for j in range(n):
    #         if np.all(all_points[j, inds1] == x_1):
    #             p_here = np.append(p_here, all_points[j:j+1, inds2], axis=0)
    #             w_here.append(all_weights[j])
    #     w_here = np.array(w_here)
    #     if np.sum(w_here) > 0:
    #         w_here = w_here/np.sum(w_here)
    #     cond_measures.append([p_here.copy(), w_here.copy()])

    return marg_inds1, cond_measures, supp_1, supp_2


def get_full_disintegration(marg_mu, g, nodes, tol=10**-6):
    # marg_mu is a d-dimensional measure in marginal form
    # g is a graph structure
    # tol specifies that below this threshold, two points are assumed to be "the same point"
    # should return mu, supp_mu according to graph structure g...
    # note that this means we are only looking at the canonical filtration according to g.
    # TODO: if the process at hand is not measurable w.r.t. g, this is a problem.
    mu_dic = {}
    supp_mu_dic = {}
    node_ids = list(range(len(nodes)))
    for idh, node in enumerate(nodes):
        pa_n = g.parents[node]
        len_pa = len(pa_n)
        all_inds = tuple(sorted([node] + pa_n))
        real_inds = []
        for testih, testnh in enumerate(nodes):
            if testnh in all_inds:
                real_inds.append(testih)
        real_inds = tuple(sorted(real_inds))

        rel_meas_p = marg_mu[0][:, real_inds]
        w_h = marg_mu[1]
        rel_meas = reduce_meas([rel_meas_p, w_h])

        nmh, cond_m, s1h, s2h = get_disintegration(rel_meas, [i + 1 for i in range(len_pa)])
        # nmh is marginal distribution of parents,
        # cond_m are conditional measures for each value of parents,
        # s1h is support of the parents,
        # and s2h is support of the children
        if len(s1h) == 0:
            mu_dic[(node, 0)] = cond_m[0]
        for ind_supp, p_supp in enumerate(s1h):
            mu_dic[(node, ind_supp)] = cond_m[ind_supp]
        supp_mu_dic[tuple(sorted(pa_n))] = s1h
        supp_mu_dic[tuple([node])] = s2h

    eps = tol

    def mu(node, x_par):
        if len(x_par) == 0:
            return mu_dic[(node, 0)]
        x_par = np.array(x_par)
        if len(x_par.shape) == 1:
            x_par.reshape(-1, 1)

        ind_rel_here = -1
        panode = g.parents[node]
        for j in range(len(supp_mu_dic[tuple(sorted(panode))])):
            if np.all(np.abs(x_par - supp_mu_dic[tuple(sorted(panode))][j, :]) < eps):
                ind_rel_here = j
                break
        if ind_rel_here == -1:
            print("ERROR: relevant support point not found...")

        return mu_dic[(node, ind_rel_here)]

    def supp_mu(node_list):
        return supp_mu_dic[tuple(sorted(node_list))]

    return mu, supp_mu


def combine_markov(meas_list):
    # meas_list is a list of measures [[mu1, suppmu1], ..., [muT, suppmuT]], each being a measure with two nodes
    # The input are functions mu_i \otimes K_i, and the output is mu_1 \otimes K_1 \otimes K_2 \otimes ... \otimes K_T
    # mu1 has nodes [0, 1], mu2 has nodes [1, 2], etc.
    # we need that suppmu_t([t]) = suppmu_{t+1}([t])

    T = len(meas_list)

    # check whether the supports of the respective measures fit together
    for t in range(T - 1):
        smt = meas_list[t][1]([t + 1])
        smtp = meas_list[t + 1][1]([t + 1])
        for x in smt:
            assert x in smtp
        for y in smtp:
            assert y in smt
    ...
    def supp_mu(node_list):
        # Only defined for single nodes, which should be enough for Markovian measures
        if len(node_list) > 1:
            print("ERROR! For the Markovian measure, we can only read out the support for single points in time")
            return 0
        if len(node_list) == 0:
            return meas_list[0][1](node_list)
        else:
            nh = node_list[0]
            if nh == 0:
                return meas_list[nh][1](node_list)
            else:
                return meas_list[nh - 1][1](node_list)

    def mu(node, x_par):
        if node == 0:
            return meas_list[0][0](node, x_par)
        else:
            return meas_list[node - 1][0](node, x_par)
...
if __name__ == "__main__":
    import numpy as np
    from awd.mainfunctions import solve_dynamic
    from awd.mainfunctions import gurobi_bm
    from awd.normal_ot import gurobi_2d
    import ot
    ...
    mu1, supmu1 = binomial()
    mu2, supmu2 = binomial()
    mu, suppmu = combine_markov([[mu1, supmu1], [mu2, supmu2]])
