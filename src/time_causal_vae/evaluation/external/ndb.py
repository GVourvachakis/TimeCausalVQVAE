"""Optional NDB diagnostic copied from the upstream evaluation code.

The NDB implementation is borrowed/adapted from the upstream reference to
`gans-n-gmms`. It is not required for selected checkpoint reproduction and should be
...
def compare_NDB(real_data, fake_data, k=6, verbose=True):
    train_samples = torch.flatten(real_data, start_dim=1).numpy()
    test_samples = torch.flatten(fake_data, start_dim=1).numpy()
    test_ndb = NDB(training_data=train_samples, number_of_bins=k, whitening=True)
    results = test_ndb.evaluate(test_samples, model_label="Test")
    test_ndb.plot_results(models_to_plot=["Test"])
    return test_ndb


def compare_clusters(real_data, fake_data, k=9):
    train_samples = torch.flatten(real_data, start_dim=1).numpy()
    test_samples = torch.flatten(fake_data, start_dim=1).numpy()
    test_ndb = NDB(training_data=train_samples, number_of_bins=k, whitening=True)
    labels = test_ndb.fit(test_samples)
    n_row = math.ceil(k / 3)
    n_col = 3
    fig, ax = plt.subplots(n_row, n_col, figsize=[16, n_row * 4])
    for i in range(k):
        axi = ax.flat[i]
        cluster_paths = test_samples[labels == i]
        axi.plot(cluster_paths[:500].T, color="C" + str(i), alpha=0.05)
        center_path = cluster_paths.mean(axis=0)
        axi.plot(center_path, "--", color="C" + str(i), linewidth=2)
        axi.set_title("cluster: " + str(i + 1))
...
class NDB:
    ...
    def construct_bins(self, training_samples, bins_file):
        ...
        clusters = KMeans(n_clusters=k, algorithm="lloyd", max_iter=100).fit(whitened_samples[:, self.used_d_indices])
    ...
    def fit(self, query_samples, model_label=None):  # modified from __calculate_bin_proportions
