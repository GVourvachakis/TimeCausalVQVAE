...
import tqdm
import copy
import time_causal_vae.evaluation.external.optimal_stopping.payoffs.payoff as po
...
class Finite_Difference_Pricer:
    ...
    def set_vol_and_div(self):
        if self.model.nb_stocks > 1 and isinstance(self.payoff, po.Put1Dim):
            ...
            self.dividend = self.model.dividend + self.model.volatility**2 / 2 - self.vol**2 / 2
        elif self.model.nb_stocks == 1:
            self.vol = self.model.volatility
            self.dividend = self.model.dividend
        else:
            raise ValueError(
                "Finite differences methods are not implemented for " "nb_stocks>1 if the payoff is not Put"
            )

    def get_central_derivative(
        self, spot, eps, compute_price=True, price_p=None, price_m=None, return_prices_pm=False
    ):
        price = None
        if compute_price:
    ...
    def compute_gamma_via_PDE(self, price, delta, theta):
        if self.model.name == "BlackScholes":
            ...
            if nb_stocks > 1 and isinstance(payoff_fct, po.Put1Dim):
                ...
                raise ValueError("Binomial Pricer is not implemented for nb_stocks>1 " "if the payoff is not Put")
    ...
    def price_and_greeks(self, eps=0.01, greeks_method="central", fd_compute_gamma_via_PDE=True, **kwargs):
        orig_spot = copy.copy(self.model.spot)
        if greeks_method == "central":
            price, delta, p_p, p_m = self.get_central_derivative(spot=orig_spot, eps=eps / 2, return_prices_pm=True)
