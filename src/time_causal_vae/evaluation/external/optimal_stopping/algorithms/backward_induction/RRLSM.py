...
class ReservoirRNNLeastSquarePricer(backward_induction_pricer.AmericanOptionPricer):
    def __init__(
        self,
        model,
        payoff,
        ...
        super().__init__(
            model, payoff, use_rnn=True, train_ITM_only=train_ITM_only, use_payoff_as_input=use_payoff_as_input
        )
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.RNN = randomized_neural_networks.randomRNN(
            state_size=state_size, hidden_size=hidden_size, factors=factors, extend=True
        )
    ...
    def calculate_continuation_value(self, values, immediate_exercise_value, h):
        ...
        reg_input = np.concatenate([h, np.ones((len(h), 1))], axis=1)
        coefficients = np.linalg.lstsq(reg_input[in_the_money[0]], values[in_the_money[0]], rcond=None)
        return_values[in_the_money_all[0]] = np.dot(reg_input[in_the_money_all[0]], coefficients[0])
...
class ReservoirRNNLeastSquarePricer2(ReservoirRNNLeastSquarePricer):
    def __init__(
        ...
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.RNN = randomized_neural_networks.randomRNN(
            state_size=state_size, hidden_size=hidden_size, factors=factors, extend=False
        )
