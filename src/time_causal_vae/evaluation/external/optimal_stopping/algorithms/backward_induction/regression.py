...
class LeastSquares(Regression):
    ...
    def calculate_regression(self, X, Y, in_the_money, in_the_money_all):
        ...
        coefficients = np.linalg.lstsq(reg_vect_mat[in_the_money[0]], Y[in_the_money[0]], rcond=None)
        continuation_values = np.dot(reg_vect_mat[in_the_money_all[0]], coefficients[0])
...
class ReservoirLeastSquares(Regression):
    def __init__(self, state_size, hidden_size=10):
        ...
        self.reservoir = randomized_neural_networks.Reservoir(hidden_size, self.state_size)
...
class ReservoirLeastSquares2(Regression):
    def __init__(self, state_size, hidden_size=10, factors=(1.0,), activation=torch.nn.LeakyReLU(0.5), reinit=False):
        self.nb_base_fcts = hidden_size + 1
        self.state_size = state_size
        self.reinit = reinit
    ...
    def calculate_regression(
        self, X_unsorted, Y, in_the_money, in_the_money_all, coefficients=None, return_coefficients=False
    ):
        if self.reinit:
            self.reservoir.init()
        X = torch.from_numpy(X_unsorted)
        X = X.type(torch.float32)
        reg_input = np.concatenate([self.reservoir(X).detach().numpy(), np.ones((len(X), 1))], axis=1)
        if coefficients is None:
            coefficients = np.linalg.lstsq(reg_input[in_the_money[0]], Y[in_the_money[0]], rcond=None)
...
class ReservoirLeastSquaresRidge(Regression):
    ...
    def __init__(
        self, state_size, hidden_size=10, factors=(1.0,), ridge_coeff=1.0, activation=torch.nn.LeakyReLU(0.5)
    ):
        self.nb_base_fcts = hidden_size + 1
        self.state_size = state_size
        self.alpha = ridge_coeff
