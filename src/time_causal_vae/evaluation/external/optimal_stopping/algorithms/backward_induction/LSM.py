...
class LeastSquaresPricer(backward_induction_pricer.AmericanOptionPricer):
    ...
    def __init__(self, model, payoff, nb_epochs=None, nb_batches=None, train_ITM_only=True, use_payoff_as_input=False):
        ...
        self.regression = regression.LeastSquares(model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1)
...
class LeastSquarePricerRidge(LeastSquaresPricer):
    ...
    def __init__(
        ...
        self.regression = regression.LeastSquaresRidge(
            model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1, ridge_coeff=ridge_coeff
        )
