...
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.backward_induction import backward_induction_pricer
...
class DeepOptimalStopping(backward_induction_pricer.AmericanOptionPricer):
    ...
    def __init__(
        self, model, payoff, nb_epochs=20, nb_batches=None, hidden_size=10, use_path=False, use_payoff_as_input=False
    ):
        del nb_batches
        super().__init__(model, payoff, use_path=use_path, use_payoff_as_input=use_payoff_as_input)
        if self.use_path:
        ...
        self.neural_stopping = OptimalStoppingOptimization(
            state_size, model.nb_paths, hidden_size=hidden_size, nb_iters=nb_epochs
        )
        ...
        if self.use_path:
            ...
            stock_values = np.concatenate(
                [
                    stock_values,
                    np.zeros(
                        (stock_values.shape[0], stock_values.shape[1], self.model.nb_dates + 1 - stock_values.shape[2])
                    ),
                ],
                axis=-1,
            )
...
class OptimalStoppingOptimization(object):
    ...
    def __init__(self, nb_stocks, nb_paths, hidden_size=10, nb_iters=20, batch_size=2000):
        self.nb_stocks = nb_stocks
        self.nb_paths = nb_paths
        self.nb_iters = nb_iters
        self.batch_size = batch_size
        self.network = neural_networks.NetworkDOS(self.nb_stocks, hidden_size=hidden_size).double()
        self.network.apply(init_weights)

    def _Loss(self, X):
        return -torch.mean(X)

    def train_network(self, stock_values, immediate_exercise_value, discounted_next_values):
        ...
        for iteration in range(self.nb_iters):
            for batch in tdata.BatchSampler(
                ...
                with torch.set_grad_enabled(True):
                    ...
                    values = immediate_exercise_value[batch].reshape(-1) * outputs + discounted_next_values[batch] * (
                        ones[batch] - outputs
                    )
