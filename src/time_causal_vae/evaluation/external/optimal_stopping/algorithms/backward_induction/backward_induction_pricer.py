...
import numpy as np
import time
import time_causal_vae.evaluation.external.optimal_stopping.data.stock_model as stock_model
import time_causal_vae.evaluation.external.optimal_stopping.algorithms.utils.utilities as utilities
...
class AmericanOptionPricer:
    ...
    def stop(
        ...
        if self.use_rnn:
            continuation_values = self.calculate_continuation_value(
                discounted_next_values, immediate_exercise_values, h
            )
        else:
            if self.use_var:
    ...
    def price(self, train_eval_split=2):
        ...
        disc_factor = np.exp((-model.rate) * model.maturity / (model.nb_dates))
        ...
        return (
            max(payoff_0, np.mean(values[self.split :]) * disc_factor),
            time_for_path_gen,
        )
    ...
    def price_upper_lower_bound(self, verbose=0, train_eval_split=2):
        ...
        for i, date in enumerate(range(stock_paths.shape[2] - 2, -1, -1)):
            ...
            M_diff[:, date + 1] = copy.deepcopy(np.maximum(payoffs[:, date + 1], prev_cont_val) - continuation_value)
    ...
    def get_spot_derivative(self, spot, eps, dW, fd_freeze_exe_boundary=True, train_eval_split=2):
        """
        ...
        disc_factor = np.math.exp((-self.model.rate) * self.model.maturity / (self.model.nb_dates))
    ...
    def get_central_derivative(
        ...
        price = [
            np.mean(values[self.split :]),
            np.mean(values_p[self.split :]),
    ...
    def get_time_derivative(self, eps, dW, fd_freeze_exe_boundary=False, train_eval_split=2):
        """
        ...
        stock_paths_p, var_paths_p = self.model.generate_paths(dW=dW * np.sqrt(self.model.dt) / np.sqrt(dt_old))
        disc_factor_p = np.math.exp((-self.model.rate) * self.model.maturity / self.model.nb_dates)
        self.model.maturity = maturity_old + eps
        self.model.dt = self.model.maturity / self.model.nb_dates
        stock_paths_m, var_paths_m = self.model.generate_paths(dW=dW * np.sqrt(self.model.dt) / np.sqrt(dt_old))
        disc_factor_m = np.math.exp((-self.model.rate) * self.model.maturity / self.model.nb_dates)
    ...
    def get_rate_derivative(self, eps, dW, fd_freeze_exe_boundary=False, train_eval_split=2):
        """
    ...
    def get_vola_derivative(self, eps, dW, fd_freeze_exe_boundary=False, train_eval_split=2):
        """
    ...
    def price_and_greeks(
        ...
        if greeks_method == "central":
            ...
            price, delta, gamma, t1 = self.get_regression(spot=orig_spot, eps=reg_eps, d=poly_deg, dW=dW)
        else:
            raise NotImplementedError
...
class EuropeanOptionPricer:
    ...
    def price(self, train_eval_split=2):
        ...
        return np.mean(payoffs[:, -1]) * disc_factor**model.nb_dates, time_for_path_gen
