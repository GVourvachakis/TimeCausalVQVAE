...
import torch
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.backward_induction import regression
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.backward_induction import LSM
...
class ReservoirLeastSquarePricer(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        self.regression = regression.ReservoirLeastSquares(state_size, hidden_size)
...
class ReservoirLeastSquarePricerFast(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        self.regression = regression.ReservoirLeastSquares2(
            state_size, hidden_size, activation=torch.nn.LeakyReLU(factors[0] / 2), factors=factors[1:]
        )
...
class ReservoirLeastSquarePricerFastSoftplus(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.regression = regression.ReservoirLeastSquares2(
            state_size, hidden_size, factors=factors, activation=torch.nn.Softplus(beta=factors[1])
        )
...
class ReservoirLeastSquarePricerFastSoftplusReinit(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.regression = regression.ReservoirLeastSquares2(
            state_size, hidden_size, factors=factors, activation=torch.nn.Softplus(beta=factors[1]), reinit=True
        )
...
class ReservoirLeastSquarePricerFastGELU(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.regression = regression.ReservoirLeastSquares2(
            state_size, hidden_size, factors=factors, activation=torch.nn.GELU()
        )
...
class ReservoirLeastSquarePricerFastSILU(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.regression = regression.ReservoirLeastSquares2(
            state_size, hidden_size, factors=factors, activation=torch.nn.SiLU()
        )
...
class ReservoirLeastSquarePricerFastELU(LSM.LeastSquaresPricer):
    ...
    def __init__(
        ...
        state_size = model.nb_stocks * (1 + self.use_var) + self.use_payoff_as_input * 1
        self.regression = regression.ReservoirLeastSquares2(
            state_size, hidden_size, factors=factors, activation=torch.nn.ELU(alpha=factors[1])
        )
