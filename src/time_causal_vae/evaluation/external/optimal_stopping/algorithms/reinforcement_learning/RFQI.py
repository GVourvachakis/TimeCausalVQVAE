...
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.reinforcement_learning import FQI
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.utils import randomized_neural_networks
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.utils import neural_networks
...
class FQI_Reservoir(reinforcement_learning_price.FQI_RL):
    ...
    def __init__(
        ...
        self.state_size = self.model.nb_stocks * (1 + self.use_var) + 2 + self.use_payoff_as_input * 1
        # +2: additional for the time and time to maturity.
        self.reservoir = randomized_neural_networks.Reservoir(self.dim_out, self.state_size)
        self.reservoir2 = randomized_neural_networks.Reservoir2(self.dim_out, self.state_size)
...
class FQI_ReservoirFast(FQI.FQIFast):
    ...
    def evaluate_bases_all(self, stock_price):
        ...
        time = torch.linspace(0, 1, stocks.shape[1]).unsqueeze(0).repeat((stocks.shape[0], 1)).unsqueeze(2)
        stocks = torch.cat([stocks, time, 1 - time], dim=-1)
        random_base = self.reservoir2(stocks)
        random_base = torch.cat([random_base, torch.ones([stocks.shape[0], stocks.shape[1], 1])], dim=-1)
...
class FQI_ReservoirFastTanh(FQI_ReservoirFast):
    ...
    def __init__(
        ...
        del nb_batches, train_ITM_only
        self.reservoir2 = randomized_neural_networks.Reservoir2(
            self.dim_out, self.state_size, factors=factors, activation=torch.nn.Tanh()
        )
...
class FQI_ReservoirFastSoftplus(FQI_ReservoirFast):
    ...
    def __init__(
        ...
        del nb_batches, train_ITM_only
        self.reservoir2 = randomized_neural_networks.Reservoir2(
            self.dim_out, self.state_size, factors=factors, activation=torch.nn.Softplus()
        )
...
class FQI_ReservoirFastRNN(FQI.FQIFast):
    ...
    def __init__(
        ...
        self.reservoir2 = randomized_neural_networks.randomRNN(
            hidden_size=self.dim_out, state_size=self.state_size, factors=factors, extend=False
        )
    ...
    def evaluate_bases_all(self, stock_price):
        ...
        random_base = torch.cat([random_base, torch.ones([random_base.shape[0], random_base.shape[1], 1])], dim=-1)
        return random_base.detach().numpy()
