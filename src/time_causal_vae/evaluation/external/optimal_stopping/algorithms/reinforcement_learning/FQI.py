...
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.utils import basis_functions
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.reinforcement_learning import reinforcement_learning_price
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.utils import utilities
from time_causal_vae.evaluation.external.optimal_stopping.run import configs
...
class FQI(reinforcement_learning_price.FQI_RL):
    ...
    def __init__(self, model, payoff, nb_epochs=20, nb_batches=None, use_payoff_as_input=False):
        super().__init__(model, payoff, nb_epochs, use_payoff_as_input=use_payoff_as_input)
    ...
    def evaluate_bases(self, stock_price, path, date, nb_dates):
        ...
        return np.array([self.bf.base_fct(i, stock_price_path_date) for i in range(self.bf.nb_base_fcts)])
...
class FQIFast(reinforcement_learning_price.FQI_RL):
    ...
    def __init__(self, model, payoff, nb_epochs=20, nb_batches=None, train_ITM_only=True, use_payoff_as_input=False):
        super().__init__(
            model, payoff, nb_epochs, train_ITM_only=train_ITM_only, use_payoff_as_input=use_payoff_as_input
        )
        self.bf = basis_functions.BasisFunctions(
            self.model.nb_stocks * (1 + self.use_var) + 2 + self.use_payoff_as_input * 1
        )
    ...
    def evaluate_bases_all(self, stock_price):
        """see base class"""
        time = np.expand_dims(
            np.repeat(np.expand_dims(np.linspace(0, 1, stock_price.shape[2]), 0), stock_price.shape[0], axis=0), 1
        )
        stocks = np.concatenate([stock_price, time, 1 - time], axis=1)
        stocks = np.transpose(stocks, (1, 0, 2))
        bf = np.concatenate(
            [np.expand_dims(self.bf.base_fct(i, stocks), axis=2) for i in range(self.bf.nb_base_fcts)], axis=2
        )
        return bf
    ...
    def price(self, train_eval_split=2):
        ...
        for epoch in range(self.nb_epochs):
            ...
            matrixU = np.tensordot(
                eval_bases[: self.split, :-1, :], eval_bases[: self.split, :-1, :], axes=([0, 1], [0, 1])
            )
            vectorV = np.sum(
                eval_bases[: self.split, :-1, :]
                * discount_factor
                * np.repeat(np.expand_dims(indicator_stop, axis=2), np.shape(eval_bases)[2], axis=2),
                axis=(0, 1),
            )
        ...
        prices = (
            np.take_along_axis(payoffs, np.expand_dims(ex_dates, axis=1), axis=1).reshape(-1)
    ...
    def price_upper_lower_bound(self, verbose=0, train_eval_split=2):
        ...
        disc_factor = np.exp((-model.rate) * model.maturity / model.nb_dates * power)
        disc_factors = np.repeat(np.expand_dims(disc_factor, axis=0), repeats=payoffs.shape[0], axis=0)
        ...
        which[:, 0] = 0
        ex_dates = np.argmax(which, axis=1)
        prices = np.take_along_axis(payoffs, np.expand_dims(ex_dates, axis=1), axis=1).reshape(-1)
        price = max(np.mean(prices[self.split :]), payoffs[0, 0])

        continuation_value[:, -1] = 0.0
        M_diff = np.zeros_like(continuation_value)
        M_diff[:, 1:] = copy.deepcopy(
            np.maximum(payoffs[:, 1:], continuation_value[:, 1:]) - continuation_value[:, :-1]
        )
    ...
    def get_central_derivative(
        ...
        stock_paths_with_payoff_p = np.concatenate([stock_paths_p, np.expand_dims(payoffs_p, axis=1)], axis=1)
        payoffs_m = self.payoff(stock_paths_m)
        stock_paths_with_payoff_m = np.concatenate([stock_paths_m, np.expand_dims(payoffs_m, axis=1)], axis=1)
        self.split = int(len(stock_paths) / train_eval_split)
        ...
        if not fd_freeze_exe_boundary:
            for epoch in range(self.nb_epochs):
                continuation_value_p = self.predict_reg(eval_bases_p[: self.split, 1:, :])
                indicator_stop = np.maximum(payoffs_p[: self.split, 1:], continuation_value_p)
                matrixU = np.tensordot(
                    eval_bases_p[: self.split, :-1, :], eval_bases_p[: self.split, :-1, :], axes=([0, 1], [0, 1])
                )
                vectorV = np.sum(
                    eval_bases_p[: self.split, :-1, :]
                    * discount_factor_p
                    * np.repeat(np.expand_dims(indicator_stop, axis=2), np.shape(eval_bases_p)[2], axis=2),
                    axis=(0, 1),
                )
                self.fit_reg(matrixU, vectorV)
            ...
            for epoch in range(self.nb_epochs):
                continuation_value_m = self.predict_reg(eval_bases_m[: self.split, 1:, :])
                indicator_stop = np.maximum(payoffs_m[: self.split, 1:], continuation_value_m)
                matrixU = np.tensordot(
                    eval_bases_m[: self.split, :-1, :], eval_bases_m[: self.split, :-1, :], axes=([0, 1], [0, 1])
                )
                vectorV = np.sum(
                    eval_bases_m[: self.split, :-1, :]
                    * discount_factor_m
                    * np.repeat(np.expand_dims(indicator_stop, axis=2), np.shape(eval_bases_m)[2], axis=2),
                    axis=(0, 1),
                )
                self.fit_reg(matrixU, vectorV)
        ...
        prices_p = payoffs_p.reshape(-1)[_ex_dates_p] * discount_factor_p**ex_dates_p
        prices_m = payoffs_m.reshape(-1)[_ex_dates_m] * discount_factor_m**ex_dates_m

        price = [np.mean(prices[self.split :]), np.mean(prices_p[self.split :]), np.mean(prices_m[self.split :])]
        derivative = np.mean((prices_p[self.split :] - prices_m[self.split :]) / (2 * eps))
    ...
    def get_regression(self, spot, eps, d, dW):
        ...
        X0 = np.random.normal(loc=spot, scale=eps, size=(self.model.nb_paths, 1))
        ...
        b_val, b_d_val, b_g_val = utilities.get_poly_basis_and_derivatives(X=np.array([[spot]]), d=d)
...
class FQIFastLaguerre(FQIFast):
    ...
    def __init__(self, model, payoff, nb_epochs=20, nb_batches=None, train_ITM_only=True, use_payoff_as_input=False):
        ...
        self.bf = basis_functions.BasisFunctionsLaguerreTime(
            self.model.nb_stocks * (1 + self.use_var) + 1 + self.use_payoff_as_input * 1, T=model.maturity
        )
        self.nb_base_fcts = self.bf.nb_base_fcts
...
class FQIFastRidge(FQIFast):
    ...
    def init_reg_model(self, ridge_coeff):
        self.reg_model = sklearn.linear_model.Ridge(alpha=ridge_coeff, fit_intercept=False)
        print("ridge")
...
class FQIFastLasso(FQIFastRidge):
    ...
    def init_reg_model(self, ridge_coeff):
        self.reg_model = sklearn.linear_model.Lasso(alpha=ridge_coeff, fit_intercept=False)
        print("lasso")
