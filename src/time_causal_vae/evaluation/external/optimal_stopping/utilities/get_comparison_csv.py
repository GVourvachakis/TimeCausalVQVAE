...
from time_causal_vae.evaluation.external.optimal_stopping.utilities import read_data
...
def get_comparison_csv(
        ...
        if len(df_) > 0:
            ...
            try:
                ...
                if len(df_) > 1:
                    ...
                    delta_std, gamma_std, theta_std, rho_std, vega_std = [None] * 5
                    price_w = "{:.4f} (--)".format(price)
                ...
                delta, delta_std, gamma, gamma_std, theta, theta_std, rho, rho_std, vega, vega_std = [None] * 10
            ...
            try:
                ...
                midprice = np.mean((df_["price_upper_bound"].values + df_["price"].values) / 2)
                std_midprice = np.std((df_["price_upper_bound"].values + df_["price"].values) / 2)
                if len(df_) > 1:
                    price_upper_w = "{:.4f} ({:.4f})".format(price_upper, std_upper)
                    midprice_w = "{:.4f} ({:.4f})".format(midprice, std_midprice)
                else:
    ...
    comp_df = pd.DataFrame(data=comp_data, columns=cols)
    if sort:
...
def combine_FD_regr_greeks_table(save_path="../latex/plots/", filters_to_add_as_columns=[]):
    save_comp_file = "{}comparison.csv".format(save_path)
    ...
    df2.rename(columns={"price_w": "price_w_reg", "delta_w": "delta_w_reg", "gamma_w": "gamma_w_reg"}, inplace=True)
...
def combine_with_dos_table(save_path="../latex/plots/", filters_to_add_as_columns=["nb_stocks", "spot"]):
    save_comp_file = "{}comparison.csv".format(save_path)
    df = pd.read_csv(save_comp_file)
    ...
    df2["dos_time_tot_w"] = df2["dos_time_tot"].apply(lambda s: "{:.0f}s".format(s))
    df2 = df2[filters_to_add_as_columns + ["dos_lower_w", "dos_upper_w", "dos_mid_w", "dos_time_tot_w"]]
...
if __name__ == "__main__":
    ...
    get_comparison_csv(filter_greeks, sort=False, filters_to_add_as_columns=filters_to_add_as_columns)
    combine_FD_regr_greeks_table(filters_to_add_as_columns=filters_to_add_as_columns)
    ...
    filters_to_add_as_columns = ["algo", "strike", "greeks_method", "hidden_size", "nb_epochs"]
    get_comparison_csv(filter_sens_rand, sort=False, filters_to_add_as_columns=filters_to_add_as_columns)
