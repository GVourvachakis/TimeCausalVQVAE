...
from time_causal_vae.evaluation.external.optimal_stopping.utilities import comparison_table as ct
from time_causal_vae.evaluation.external.optimal_stopping.utilities.comparison_table import _human_time_delta
...
def plot_hurst(
    ...
    for i, filter in enumerate(filters):
        ...
        table_dict[label] = ["{:.2f} ({:.2f})".format(x, y) for x, y in zip(price, std)]
        table_dict["{}_duration".format(label)] = [_human_time_delta(sec) for sec in times]
        ...
        plt.errorbar(hurst, price, std, label=label, color=filter["color"], linestyle=filter["linestyle"])
...
def get_hurst_table_tex(path):
    ...
    for algo in algos:
        ...
        table_app = pd.DataFrame(data={"H": hurst, "algo": [algo] * len(hurst), "price": price, "duration": dur})
    ...
    bold_algos = []
    _table_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../../latex/tables_draft/"))
    if not os.path.exists(_table_path):
        os.makedirs(_table_path)
    table_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../../../latex/tables_draft/{label}.tex"))
    mcol_format = " ".join(">{\\bfseries}r" if algo in bold_algos else "r" for algo in algos)
    ind = df.index.names
    if ind == [None]:
        col_format = "|" + "|".join([mcol_format] * len(column_names)) + "|"
    else:
        col_format = "|" + "c" * len(ind) + "|" + "|".join([mcol_format] * len(column_names)) + "|"
    ...
    for line in pdf_table.split("\n"):
        if "algo &" in line:
            ...
            line.startswith("nb\\_stocks &")
            or line.startswith("hidden\\_size &")
            or line.startswith("maturity &")
            or line.startswith("payoff ")
            or line.startswith("model ")
            or line.startswith("H ")
        ):
            continue
        elif oneline and line.startswith("{} &"):
            new_lines.append(line.replace("{} &", "").replace("{c |}{price}", "{| c |}{price}"))
