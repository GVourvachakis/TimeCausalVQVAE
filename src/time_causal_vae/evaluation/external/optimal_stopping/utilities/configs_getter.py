...
def get_configs():
    ...
    all_configs = [
        (name, conf)
        for name, conf in inspect.getmembers(configs)
    ...
    if unknown_requested_configs:
        raise AssertionError(f"--configs flag contain invalid values: {unknown_requested_configs}")
    return [(name, config) for name, config in all_configs if name in FLAGS.configs]
