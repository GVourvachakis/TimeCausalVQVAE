...
flags.DEFINE_bool(
    "debug_csv_filtering", False, "Set to True to display why rows are filtered out and a snippet of data."
)
...
def filter_df(df, config: configs._DefaultConfig, reverse_filtering: bool = False):
    ...
    for filter_name, column_name in FILTERS:
        ...
        if FLAGS.debug_csv_filtering and any(~idx):
            print(f"Filtering out {sum(~idx)} rows because {column_name} " f"not in {values}")
            print(df[~idx])
        df = df[idx]
    return df
