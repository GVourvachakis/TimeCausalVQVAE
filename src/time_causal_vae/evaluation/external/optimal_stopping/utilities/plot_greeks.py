...
def plot_greeks(
    ...
    for n, plot_label in enumerate(["price", "delta", "gamma", "theta", "rho", "vega"]):
        ...
        for v in volatilities:
            for m in maturities:
                if len(df.loc[((df["volatility"] == v) & (df["maturity"] == m))]) == 0:
                    continue
                vals = []
                for s in spots:
                    _df = df.loc[((df["volatility"] == v) & (df["maturity"] == m) & (df["spot"] == s)), plot_label]
                    which = _df.abs() > 500
    ...
    plt.savefig("{}greeks_plot_{}_{}.pdf".format(save_path, algo, greeks_method), **save_extras)
