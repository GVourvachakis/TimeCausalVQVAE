...
def write_table_price(label: str, config: configs._DefaultConfig, get_df=False):
    df = _write_table_for(label, config, ["price"], "Prices returned by each algo", get_df=get_df)
    if get_df:
        return df
...
def _human_time_delta(delta):
    ...
    return "".join(["%dh" % hours if hours else "  ", "%2dm" % minutes if minutes else "  ", "%2ds" % seconds])
...
def _write_table_for(
    ...
    get_max_usepayoff=False,
    get_algo_specific_usepayoff=True,
):
    ...
    if "price" in column_names:
        if get_df:
            ...
            df["price"] = ["%.2f (%.2f)" % ms for ms in zip(df["mean_price"], df["std_price"])]
