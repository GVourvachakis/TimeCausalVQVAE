...
from time_causal_vae.evaluation.external.optimal_stopping.utilities.comparison_table import USE_PAYOFF_FOR_ALGO
...
def plot_tables(*args, **kwargs):
    ...
    for config_name, config in configs_getter.get_configs():
        ...
        index = ["algo", "payoff", "nb_paths", "nb_dates", "spot", "nb_stocks", "model", "use_payoff_as_input"]
        ...
        for nbpaths, payoff, nbdates, spot, model in itertools.product(
            ...
            for i, algo in enumerate(algos_):
                ...
                for d in all_nbstocks:
                    try:
                        prices.append(
                            mean_price[(algo, payoff, nbpaths, nbdates, spot, d, model, USE_PAYOFF_FOR_ALGO[algo])]
                        )
                        err.append(std[(algo, payoff, nbpaths, nbdates, spot, d, model, USE_PAYOFF_FOR_ALGO[algo])])
                        time.append(
                            median_duration[
                                (algo, payoff, nbpaths, nbdates, spot, d, model, USE_PAYOFF_FOR_ALGO[algo])
                            ]
                        )
                        stocks.append(d)
                    except ValueError:
                ...
                if len(prices) > 0:
                    ax[0].errorbar(x=stocks, y=prices, yerr=err, label=algo, color=color, linestyle=lst)
                    ax[1].plot(stocks, time, label=algo, color=color, linestyle=lst)
                else:
