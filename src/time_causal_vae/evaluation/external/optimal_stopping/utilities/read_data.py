...
from time_causal_vae.evaluation.external.optimal_stopping.utilities import filtering
...
def get_csv_paths():
    ...
    return [os.path.join(csvs_dir, fname) for fname in sorted(os.listdir(csvs_dir)) if fname.endswith(".csv")]
...
def read_csv(path: str, config: configs._DefaultConfig, reverse_filtering: bool = False):
