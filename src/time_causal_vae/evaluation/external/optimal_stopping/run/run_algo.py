...
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.backward_induction import (
    DOS,
    LSM,
    NLSM,
    RLSM,
...
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.finite_difference import binomial, trinomial
from time_causal_vae.evaluation.external.optimal_stopping.algorithms.reinforcement_learning import FQI, LSPI, RFQI
from time_causal_vae.evaluation.external.optimal_stopping.data import stock_model
from time_causal_vae.evaluation.external.optimal_stopping.payoffs import payoff
from time_causal_vae.evaluation.external.optimal_stopping.run import configs, write_figures
from time_causal_vae.evaluation.external.optimal_stopping.utilities import configs_getter
...
flags.DEFINE_bool("compute_upper_bound", False, "Whether to additionally compute upper bound for price")
flags.DEFINE_bool("compute_greeks", False, "Whether to compute greeks (not available for all settings)")
flags.DEFINE_string("greeks_method", "central", "one of: central, forward, backward, regression")
flags.DEFINE_float("eps", 1e-9, "the epsilon for the finite difference method or regression")
flags.DEFINE_float("reg_eps", 5, "the epsilon for the finite difference method or regression")
flags.DEFINE_integer("poly_deg", 2, "the degree for the polynomial regression")
flags.DEFINE_bool("fd_freeze_exe_boundary", True, "Whether to use same exercise boundary")
flags.DEFINE_bool("fd_compute_gamma_via_PDE", True, "Whether to use the PDE to compute gamma")
flags.DEFINE_bool("DEBUG", False, "Turn on debug mode")
flags.DEFINE_integer("train_eval_split", 2, "divisor for the train/eval split")
...
def _run_algos():
    fpath = os.path.join(os.path.dirname(__file__), "../../output/metrics_draft", f"{int(time.time()*1000)}.csv")
    ...
    for config_name, config in configs_getter.get_configs():
        ...
        config.algos = [a for a in config.algos if FLAGS.algos is None or a in FLAGS.algos]
        if nb_stocks_flag:
            config.nb_stocks = [a for a in config.nb_stocks if a in nb_stocks_flag]
        combinations = list(
            itertools.product(
                config.algos,
                config.dividends,
                ...
                config.factors,
                config.ridge_coeff,
                config.train_ITM_only,
                config.use_path,
                config.use_payoff_as_input,
            )
        )
                ...
                delayed_jobs.append(
                    joblib.delayed(_run_algo)(
                        ...
                        *params,
                        ...
                        path_gen_seed=FLAGS.path_gen_seed,
                        compute_upper_bound=FLAGS.compute_upper_bound,
                        DEBUG=FLAGS.DEBUG,
                        train_eval_split=FLAGS.train_eval_split,
                    )
                )

    print(f"Running {len(delayed_jobs)} tasks using " f"{FLAGS.nb_jobs}/{NUM_PROCESSORS} CPUs...")
...
def _run_algo(
    ...
    if algo in ["NLSM"]:
        ...
        pricer = _ALGOS[algo](stock_model_, payoff_, nb_epochs=nb_epochs, use_payoff_as_input=use_payoff_as_input)

    t_begin = time.time()

    if DEBUG:
        if compute_upper_bound:
            price, price_u, gen_time = pricer.price_upper_lower_bound(train_eval_split=train_eval_split)
    ...
    try:
        if SEND:
            SBM.send_notification(
                text="start running AMC2 with config:\n{}".format(FLAGS.configs), chat_id="-399803347"
            )
        configs.path_gen_seed.set_seed(FLAGS.path_gen_seed)
        filepath = _run_algos()
        ...
        if SEND:
            time.sleep(1)
            SBM.send_notification(text="finished", files=[filepath], chat_id="-399803347")
    except Exception as e:
        if SEND:
            SBM.send_notification(text="ERROR\n{}".format(e), chat_id="-399803347")
        else:
            print("ERROR\n{}".format(e))
